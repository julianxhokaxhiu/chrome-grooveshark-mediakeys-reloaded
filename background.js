chrome.extension.onMessage.addListener(function(request,sender,sendResponse){
	...
	if(request.type||request.cmd){
		if(request.type=='tabid')chrome.windows.getCurrent(function(win){chrome.tabs.query({'windowId':win.id,'active':true},function(t){if(t){sendResponse({isdz:(t[0].url.search('grooveshark.com')>0)})}})});//chrome.tabs.getSelected(null,function(t){sendResponse({isdz:(t.url.search('www.deezer.com')>0)})});
		else if(request.cmd){
			chrome.windows.getAll({populate:true},function(w){
				$.each(w,function(i,v){
					$.each(v.tabs,function(i,v){
						if((!found)&&(v.url.search('grooveshark.com')>0)){
							found=true;
