chrome.extension.onMessage.addListener(function(request,sender,sendResponse){
	...
	if(request.type||request.cmd){
		if(request.type=='tabid')chrome.windows.getCurrent(function(win){chrome.tabs.query({'windowId':win.id,'active':true},function(t){if(t){sendResponse({isgs:(t[0].url.search('grooveshark.com')>0)})}})});
		else if(request.cmd){
